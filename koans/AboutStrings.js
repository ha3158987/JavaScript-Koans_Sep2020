...
describe("JavaScript String(문자열) 알아보기", function() {
  ...
  it("문자열의 type 알아보기", function () {
    var str = "문자열입니당";
    expect(typeof(str)).toBe(FILL_ME_IN);
  });
  ...
  it("문자열의 length 속성 알아보기", function () {
    var str1 = "안녕하세요";
    expect(str1.length).toBe(FILL_ME_IN);

    var str2 = "안녕";
    expect(str2.length).toBe(FILL_ME_IN);

    var str3 = "";
    expect(str3.length).toBe(FILL_ME_IN);

    var str4 = " ";
    expect(str4.length).toBe(FILL_ME_IN);

    var str5 = "  ";
    expect(str5.length).toBe(FILL_ME_IN);
  });

  it("문자열의 인덱스 알아보기", function() {
    var message = "안녕, 내 이름은 돈까스라고 해.";

    expect(message[0]).toBe(FILL_ME_IN);
    expect(message[message.length - 1]).toBe(FILL_ME_IN);
  });
  ...
  it("문자열 더하기", function () {
    ...
    expect(lastName + firstName).toEqual(FILL_ME_IN);
  });
  ...
  it("템플릿 리터럴 방식으로 문자열 만들기", function () {
    ...
    expect(orderMessage).toEqual(FILL_ME_IN);
  });

  it("문자열 자르기", function () {
    var orderMessage = "돈까스 3인분 주세요.";

    expect(orderMessage.substr(0, 3)).toEqual(FILL_ME_IN);
    expect(orderMessage.substr(4, 1)).toEqual(FILL_ME_IN);
  });

  it("특정 문자를 기준으로 자르기", function () {
    var orderMessage = "돈까스, 돈까스, 돈까스";

    expect(orderMessage.split(", ")).toEqual(FILL_ME_IN);
    expect(orderMessage.split("")).toEqual(FILL_ME_IN);
  });
  ...
  it("문자열 내에서 특정 문자 찾기", function () {
    ...
    expect(orderMessage.indexOf("돈까스")).toEqual(FILL_ME_IN);

    orderMessage = "돈까스 3인분 주세요. 앗, 죄송한데 돈까스 1인분 추가요.";

    expect(orderMessage.indexOf("돈까스")).toEqual(FILL_ME_IN);
    expect(orderMessage.indexOf(3)).toEqual(FILL_ME_IN);
    expect(orderMessage.lastIndexOf("돈까스")).toEqual(FILL_ME_IN);
  });
  ...
  it("문자열 순회하기", function () {
    ...
    expect(greeting1).toEqual(FILL_ME_IN);
    expect(greeting2).toEqual(FILL_ME_IN);
  });
});
